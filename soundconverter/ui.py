...
from soundconverter.settings import (
...
class FileList:
    ...
    def add_uris(self, uris, base=None, extensions=None):
                    ...
                    for f in filelist:
                        for extension in extensions:
                            if f.lower().endswith(extension):
                                final_path = urllib.parse.unquote(
                                    uri.replace("file://", "")
                                )
        ...
        if ignored_files:
            self.dialog.show_warning(
                "One or more audio files weren't imported because of the size (> 2GB)",
    ...
    def format_cell(self, sound_file):
        return "%s" % GObject.markup_escape_text(unquote_filename(sound_file.filename))
...
class PreferencesDialog(GladeWindow, GConfStore):
    ...
    def set_widget_initial_values(self, builder):
        ...
        assert len(model) == len(widgets), "model:%d widgets:%d" % (
            len(model),
            len(widgets),
        )
    ...
    def update_selected_folder(self):
        ...
        self.into_selected_folder.set_label(
            _("Into folder %s") % beautify_uri(self.get_string("selected-folder"))
        )
    ...
    def update_example(self):
        ...
        s = GObject.markup_escape_text(
            beautify_uri(self.generate_filename(sound_file, for_display=True))
        )
        p = 0
        replaces = []
        ...
        markup = "<small>%s</small>" % (
            _("Target bitrate: %s") % self.get_bitrate_from_settings()
        )
    ...
    def on_delete_warning_button_clicked(self, button):
        ...
        if self.get_int("delete-original"):
            ...
            self.wdialog.show_warning(
                "",
                "Deleting the original file is potentially destructive, mainly if the conversion is towards a lossy format.",
            )
    ...
    def on_output_mime_type_changed(self, combo):
        self.change_mime_type(self.present_mime_types[combo.get_active()])
...
class SoundConverterWindow(GladeWindow):
    ...
    def set_progress(self, fraction=None, display_time=True):
        ...
        if display_time:
            t = time.time() - self.converter.run_start_time - self.paused_time
            if t < 1:
